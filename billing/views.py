...
from .mikrotik import (
    ...
    deactivate_user_other_devices,  # Last Device Wins
    # Multi-tenant functions
    authenticate_user_on_tenant_routers,
    revoke_user_access_on_tenant_routers,
    grant_user_access_on_router,
...
def system_settings(request):
    """
    Get or update system settings for Platform Admin (Admin only)
    GET: Retrieve current system configuration including multi-tenant settings
    PUT: Update system settings (requires server restart for Django settings changes)

    This endpoint provides comprehensive platform administration settings including:
    - Global MikroTik settings (legacy/default router)
    - Payment gateway settings (ClickPesa)
    - SMS gateway settings (NextSMS)
        ...
        try:
            ...
            settings_data = {
                ...
                "nextsms": {
                    ...
                    "base_url": getattr(django_settings, "NEXTSMS_BASE_URL", ""),
                },
                "system": {
                ...
                plans_data.append(
                    {
                        ...
                        "yearly_price": (
                            float(plan.yearly_price) if plan.yearly_price else None
                        ),
                        "max_wifi_users": plan.max_wifi_users,
            ...
            settings_data["subscription_plans"] = plans_data

            # Add tenant summary
            tenant_summary = {
                "total": Tenant.objects.count(),
                "active": Tenant.objects.filter(is_active=True).count(),
                "suspended": Tenant.objects.filter(is_active=False).count(),
                "by_plan": {},
            }
            # Count tenants per plan
            for plan in SubscriptionPlan.objects.all():
                count = Tenant.objects.filter(subscription_plan=plan).count()
                tenant_summary["by_plan"][plan.name] = count
            settings_data["tenant_summary"] = tenant_summary

            # Add router summary
            router_summary = {
                "total": Router.objects.count(),
                "active": Router.objects.filter(is_active=True).count(),
                "online": Router.objects.filter(status="online").count(),
                "offline": Router.objects.filter(status="offline").count(),
            }
            settings_data["router_summary"] = router_summary

            # Add recent activity
            now = timezone.now()
            today = now.date()
            settings_data["activity"] = {
                "new_tenants_today": Tenant.objects.filter(
                ...
                "subscription_payments_today": TenantSubscriptionPayment.objects.filter(
                    created_at__date=today, status="completed"
                ).count(),
            }
...
def system_status(request):
    """
    Get overall system health and status for ALL tenants (Platform Admin only)
    Shows aggregate statistics across all tenants
    """
    try:
        from django.db import connection
        ...
        # MikroTik connection test - check all active routers
        ...
        try:
            ...
            routers_offline = total_routers - routers_online
            mikrotik_status = "OK" if routers_online > 0 else "NO_ROUTERS"
        except Exception:
            mikrotik_status = "ERROR"

        # Get system statistics across ALL tenants
        now = timezone.now()
        today = now.date()
        ...
        stats = {
            ...
            # Payment stats - WiFi payments from ALL tenants
            "payments_today": Payment.objects.filter(
                created_at__date=today, status="completed"
            ).count(),
            ...
            or 0,
            # Subscription payments from tenants
            "subscription_payments_today": TenantSubscriptionPayment.objects.filter(
                created_at__date=today, status="completed"
            ).count(),
            "subscription_revenue_today": TenantSubscriptionPayment.objects.filter(
                created_at__date=today, status="completed"
            ).aggregate(total=Sum("amount"))["total"]
            or 0,
            "subscription_payments_week": TenantSubscriptionPayment.objects.filter(
                created_at__gte=week_ago, status="completed"
            ).count(),
            "subscription_revenue_week": TenantSubscriptionPayment.objects.filter(
                created_at__gte=week_ago, status="completed"
            ).aggregate(total=Sum("amount"))["total"]
            or 0,
            # Bundle and voucher stats
            "active_bundles": Bundle.objects.filter(is_active=True).count(),
            "pending_payments": Payment.objects.filter(status="pending").count(),
            "total_vouchers": Voucher.objects.count(),
            "available_vouchers": Voucher.objects.filter(is_used=False).count(),
            # Tenant stats
        ...
        for tenant in Tenant.objects.filter(is_active=True):
            tenant_data = {
                ...
                "routers": Router.objects.filter(tenant=tenant, is_active=True).count(),
                "revenue_today": Payment.objects.filter(
                    user__tenant=tenant, created_at__date=today, status="completed"
                ).aggregate(total=Sum("amount"))["total"]
                or 0,
                "vouchers_available": Voucher.objects.filter(
                    tenant=tenant, is_used=False
                ).count(),
            }
            tenant_stats.append(tenant_data)

        stats["by_tenant"] = tenant_stats
...
def expiry_watcher_status(request):
    ...
    try:
        ...
        # GET - Return status for ALL tenants
        watcher = get_watcher()

        # Get users about to expire (within 30 minutes) from ALL tenants
        expiring_soon = (
            User.objects.filter(
                is_active=True,
                paid_until__gt=now,
        ...
        # Get recently expired (should be empty if watcher is working) from ALL tenants
        recently_expired = (
            User.objects.filter(is_active=True, paid_until__lte=now)
            .select_related("tenant")
            .values(
                "id",
                "phone_number",
            ...
            expiring_list.append(
                {
                    "id": user["id"],
                    "phone_number": user["phone_number"],
                    "tenant": user["tenant__slug"],
                    "tenant_name": user["tenant__business_name"],
        ...
        tenant_stats = {}
        for tenant in Tenant.objects.filter(is_active=True):
            tenant_active = User.objects.filter(
                tenant=tenant, is_active=True, paid_until__gt=now
            ).count()
            tenant_expiring = User.objects.filter(
                tenant=tenant,
                is_active=True,
                paid_until__gt=now,
                paid_until__lte=now + timedelta(minutes=30),
            ).count()
            tenant_expired = User.objects.filter(
                tenant=tenant, is_active=True, paid_until__lte=now
            ).count()
            tenant_stats[tenant.slug] = {
                "tenant_name": tenant.business_name,
                "active_users": tenant_active,
                "expiring_soon": tenant_expiring,
                "expired_not_disconnected": tenant_expired,
            }
...
def admin_list_all_routers(request):
    ...
    try:
        ...
        check_connectivity = (
            request.query_params.get("check_connectivity", "false").lower() == "true"
        )
    ...
    try:
        ...
        if not api:
            return Response(
                {
                    "success": False,
                    "message": f"Cannot connect to router {router.name}",
                },
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
            ...
            for session in active:
                ...
                try:
                    db_user = User.objects.get(
                        phone_number=username, tenant=router.tenant
                    )
                    db_info = {
                        "user_id": db_user.id,
                        ...
                        "access_expires_at": (
                            db_user.paid_until.isoformat()
                            if db_user.paid_until
                            else None
                        ),
                    }
                except User.DoesNotExist:
...
def verify_access(request):
    ...
    router_id = serializer.validated_data.get("router_id")  # New: specific router
    ...
    router = None  # Initialize router for later use in AccessLog
    ...
    # If router_id provided, get router and tenant
    if router_id:
        try:
    ...
    try:
        ...
        if not has_access:
            ...
            try:
                ...
                if device_tracking_result["success"]:
                    # Capture device switch info if applicable
                    if device_tracking_result.get("device_switch"):
                        device_switch_info = device_tracking_result["device_switch"]
                        logger.info(
                            f"LAST DEVICE WINS: {phone_number} switched devices. "
                            f"Deactivated {device_switch_info.get('deactivated_count', 0)} old device(s)"
                        )
                ...
                # Fallback to original device tracking method with "Last Device Wins"
                try:
                    # LAST DEVICE WINS: Deactivate other devices first
                    from .mikrotik import deactivate_user_other_devices
                    ...
                    if other_active_devices.exists():
                        logger.info(
                            f"FALLBACK - LAST DEVICE WINS: Deactivating {other_active_devices.count()} "
                            f"other device(s) for {phone_number}"
                        )
                        deactivate_user_other_devices(user, mac_address, ip_address)
                    ...
                    if not created:
                        ...
                        # New device - with "Last Device Wins", this is now the only active device
                        logger.info(
                            f"New device registered for {phone_number}: {mac_address} (Last Device Wins, method: {access_method})"
                        )
        ...
        if has_access and mac_address:
            ...
            try:
                logger.info(
                    f"✓ User {phone_number} has valid access - connecting to MikroTik (MAC: {mac_address}, IP: {ip_address}, router_id: {router_id})"
                )

                # IMPORTANT: If router_id is provided, authorize ONLY on that specific router
                # This is crucial for multi-tenant SaaS to ensure tenant isolation
                if router_id:
                    # Authorize on specific router only
                    auto_login_result = authorize_user_on_specific_router(
                        user=user,
                        router_id=router_id,
                        mac_address=mac_address,
                        ip_address=ip_address,
                    ...
                    # No specific router - authorize on ALL tenant routers (for trigger-auth, etc.)
                    auto_login_result = (
                        force_immediate_internet_access_on_tenant_routers(
                            user=user,
                            mac_address=mac_address,
                    ...
                    # Fallback to global router for legacy users without tenant
                ...
                if auto_login_result.get("success"):
                    ...
                    logger.info(
                        f'✓ Automatically connected {phone_number} to internet via MikroTik (method: {auto_login_result.get("method_used")}, router_id: {router_id or "all"})'
                    )
                else:
            ...
            try:
                ...
                # IMPORTANT: If router_id is provided, revoke ONLY on that specific router
                if router_id:
                    revoke_result = revoke_user_on_specific_router(
                        user=user, router_id=router_id, mac_address=mac_address
                    )
                elif user.tenant:
                    # Revoke access on ALL of tenant's routers
                    revoke_result = revoke_user_access_on_tenant_routers(
                        user=user, mac_address=mac_address
                    )
                else:
                    # Fallback to global router for legacy users without tenant
        ...
        logger.info(
            f'Creating AccessLog: user={user.phone_number}, ip={ip_address}, mac={mac_address}, access_granted={has_access}, device={device.device_name if device else "None"}, router={router.name if router else "None"}'
        )

        AccessLog.objects.create(
            tenant=tenant,  # Explicit tenant association
            user=user,
            device=device,
            router=router,  # Track which router the access was verified on
        ...
        response_data = {
            ...
            # Device switch info (Last Device Wins feature)
            "device_switch": device_switch_info,
        }
...
def initiate_payment(request):
    ...
    # =========================================================================
    # MULTI-TENANT: Get router object if router_id provided
    # This is crucial for knowing WHERE to grant access after payment
    # =========================================================================
    router = None
    if router_id:
        try:
            router = Router.objects.get(id=router_id, is_active=True)
            # If we got tenant from router, ensure user is associated with this tenant
    ...
    payment = Payment.objects.create(
        ...
        router=router,  # Associate payment with specific router (for targeted access grant)
        user=user,
        bundle=bundle,
        amount=amount,
        phone_number=phone_number,
...
def _handle_ppp_billpay_webhook(
    webhook_data,
    webhook_log,
    order_reference,
    transaction_id,
    ...
    try:
        ...
        if bill_pay_number:
            try:
                ...
                customer = PPPCustomer.objects.filter(
                    control_number=bill_pay_number
                ).first()
        ...
        if event_type == "PAYMENT RECEIVED" or status_code in [
            "SUCCESS",
            "COMPLETED",
            "PAYMENT RECEIVED",
        ]:
            ...
            payment_amount = (
                Decimal(str(amount)) if amount else customer.effective_price
            )
            ...
            try:
                ...
                if customer.phone_number:
                    ...
                    plan_name = (
                        customer.plan.name if customer.plan else customer.profile.name
                    )
            ...
            return Response(
                {
                    "success": True,
                    "message": "PPP BillPay payment processed successfully",
                }
            )
            ...
            failed_order_ref = (
                f"PPPBILL-FAIL-{customer.id}-{_uuid.uuid4().hex[:8].upper()}"
            )
        ...
        if success:
            ...
            if mac_address:
                try:
                    ...
                    # Fallback to original device registration method with "Last Device Wins"
                    try:
                        # LAST DEVICE WINS: Deactivate other devices first
                        from .mikrotik import deactivate_user_other_devices

                        other_active_devices = user.devices.filter(
                            is_active=True
                        ).exclude(mac_address__iexact=mac_address)
                        if other_active_devices.exists():
                            logger.info(
                                f"VOUCHER FALLBACK - LAST DEVICE WINS: Deactivating {other_active_devices.count()} "
                                f"other device(s) for {phone_number}"
                            )
                            deactivate_user_other_devices(user, mac_address, ip_address)
                        ...
                        if device_created:
                            device_info = {
                                ...
                                "device_id": device.id,
                                "mac_address": mac_address,
                                "device_count": 1,  # Always 1 with Last Device Wins
                                "max_devices": user.max_devices,
                                "fallback_registration": True,
                                "last_device_wins": True,
                            }
                            logger.info(
                                f"Fallback device registration successful for voucher user {phone_number}: {mac_address} (Last Device Wins)"
                            )
                        else:
                            # Update existing device
...
def list_vouchers(request):
    ...
    for voucher in vouchers_page:
        voucher_data = {
            ...
            "tenant_name": (
                voucher.tenant.business_name if voucher.tenant else "Platform"
            ),
            "duration_hours": voucher.duration_hours,
            ...
            "batch_id": voucher.batch_id,
            "created_at": (
                voucher.created_at.isoformat() if voucher.created_at else None
            ),
    ...
    return Response(
        {
            ...
            "pagination": {
                ...
                "page_size": page_size,
                "total_pages": (
                    (total_vouchers + page_size - 1) // page_size
                    if total_vouchers > 0
                    else 0
                ),
            },
...
def trigger_device_authentication(request):
    ...
    try:
        ...
        try:
            ...
            if user.tenant:
                from .mikrotik import authenticate_user_on_tenant_routers

                mikrotik_result = authenticate_user_on_tenant_routers(
                    user=user,
                    mac_address=mac_address,
                    ip_address=ip_address,
                )
...
def list_sms_broadcasts(request):
    """
    List all SMS broadcast campaigns

    GET /api/sms-broadcasts/

    Query params:
    - status: Filter by status (draft, pending, sending, completed, failed, cancelled)
    - limit: Number of results (default 50)
    - offset: Pagination offset
    """
    from .models import SMSBroadcast

    broadcasts = SMSBroadcast.objects.all()
        ...
        data.append(
            {
                ...
                "title": b.title,
                "message": (
                    b.message[:100] + "..." if len(b.message) > 100 else b.message
                ),
                "target_type": b.target_type,
                "target_type_display": b.get_target_type_display(),
                "target_tenant": str(b.target_tenant.id) if b.target_tenant else None,
                "target_tenant_name": (
                    b.target_tenant.business_name if b.target_tenant else None
                ),
                "status": b.status,
...
@api_view(["POST"])
@permission_classes([IsAdminUser])
def create_sms_broadcast(request):
    """
    Create a new SMS broadcast campaign

    POST /api/sms-broadcasts/create/

    Body:
    {
        "title": "Welcome Campaign",
        "message": "Hello! Thanks for using our WiFi service.",
        "target_type": "all_users" | "all_tenants" | "tenant_users" | "active_users" | "expired_users" | "custom",
        "target_tenant_id": "uuid" (required if target_type is "tenant_users"),
        "custom_recipients": ["255712345678", "255787654321"] (required if target_type is "custom"),
        "send_now": true | false
    }
    """
    from .models import SMSBroadcast, Tenant

    title = request.data.get("title")
    message = request.data.get("message")
    target_type = request.data.get("target_type", "all_users")
    target_tenant_id = request.data.get("target_tenant_id")
    ...
    if len(message) > 320:
        return Response(
            {
                "success": False,
                "message": "Message too long. Maximum 320 characters allowed (2 SMS segments).",
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if target_type not in valid_types:
        return Response(
            {
                "success": False,
                "message": f'Invalid target_type. Must be one of: {", ".join(valid_types)}',
            },
            status=status.HTTP_400_BAD_REQUEST,
        )

    # Validate tenant for tenant_users target
    target_tenant = None
    if target_type == "tenant_users":
        if not target_tenant_id:
            return Response(
                {
                    "success": False,
                    "message": 'target_tenant_id is required when target_type is "tenant_users"',
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        try:
    ...
    if target_type == "custom":
        if not custom_recipients or not isinstance(custom_recipients, list):
            return Response(
                {
                    "success": False,
                    "message": 'custom_recipients list is required when target_type is "custom"',
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        if len(custom_recipients) == 0:
            return Response(
                {"success": False, "message": "custom_recipients cannot be empty"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    broadcast = SMSBroadcast.objects.create(
        ...
        custom_recipients=custom_recipients if target_type == "custom" else None,
        status="draft",
    ...
    result_message = (
        f"Broadcast created successfully. {len(recipients)} recipients found."
    )
    if send_now:
        success, send_message = broadcast.send_broadcast()
        result_message = send_message
...
@api_view(["GET"])
@permission_classes([IsAdminUser])
def get_sms_broadcast(request, broadcast_id):
    """
    Get details of a specific SMS broadcast

    GET /api/sms-broadcasts/<broadcast_id>/
    """
    from .models import SMSBroadcast, SMSLog

    try:
        broadcast = SMSBroadcast.objects.get(id=broadcast_id)
    except (SMSBroadcast.DoesNotExist, ValueError):
        return Response(
    ...
    sms_logs = SMSLog.objects.filter(
        sms_type="admin", message=broadcast.message, sent_at__gte=broadcast.created_at
    ...
    return Response(
        {
            ...
            "broadcast": {
                ...
                "target_type_display": broadcast.get_target_type_display(),
                "target_tenant": (
                    str(broadcast.target_tenant.id) if broadcast.target_tenant else None
                ),
                "target_tenant_name": (
                    broadcast.target_tenant.business_name
                    if broadcast.target_tenant
                    else None
                ),
                ...
                "created_by": (
                    broadcast.created_by.username if broadcast.created_by else None
                ),
                "created_at": (
...
@api_view(["POST"])
@permission_classes([IsAdminUser])
def send_sms_broadcast(request, broadcast_id):
    """
    Send/execute a draft SMS broadcast

    POST /api/sms-broadcasts/<broadcast_id>/send/
    """
    from .models import SMSBroadcast
...
@api_view(["DELETE"])
@permission_classes([IsAdminUser])
def delete_sms_broadcast(request, broadcast_id):
    """
    Delete an SMS broadcast (only drafts can be deleted)

    DELETE /api/sms-broadcasts/<broadcast_id>/
    """
    from .models import SMSBroadcast
    ...
    if broadcast.status != "draft":
        return Response(
            {"success": False, "message": "Only draft broadcasts can be deleted"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
@api_view(["GET"])
@permission_classes([IsAdminUser])
def preview_sms_broadcast(request):
    """
    Preview recipients for a broadcast without creating it

    GET /api/sms-broadcasts/preview/

    Query params:
    - target_type: all_users, all_tenants, tenant_users, active_users, expired_users
    - target_tenant_id: UUID (required for tenant_users)
    """
    from .models import Tenant, User as WiFiUser

    target_type = request.query_params.get("target_type", "all_users")
    ...
    if target_type == "all_users":
        users = WiFiUser.objects.filter(phone_number__isnull=False).values(
            "phone_number", "name"
        )[:20]
        recipients = list(users)
        total = WiFiUser.objects.filter(phone_number__isnull=False).count()

    elif target_type == "all_tenants":
        ...
        if target_tenant_id:
            try:
                target_tenant = Tenant.objects.get(id=target_tenant_id)
                users = WiFiUser.objects.filter(
                    tenant=target_tenant, phone_number__isnull=False
                ).values("phone_number", "name")[:20]
                recipients = list(users)
                total = WiFiUser.objects.filter(
                    tenant=target_tenant, phone_number__isnull=False
                ).count()
            except (Tenant.DoesNotExist, ValueError):
                total = 0
    ...
    elif target_type == "expired_users":
        now = timezone.now()
        users = WiFiUser.objects.filter(
    ...
    return Response(
        {
            ...
            "target_type": target_type,
            "target_tenant_name": (
                target_tenant.business_name if target_tenant else None
            ),
...
def send_single_sms(request):
    """
    Send a single SMS to a phone number (for testing or quick messages)

    POST /api/sms-broadcasts/send-single/
    ...
    if len(message) > 320:
        return Response(
            {
                "success": False,
                "message": "Message too long. Maximum 320 characters allowed.",
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    result = sms_api.send_sms(
        phone_number, message, reference=f"SINGLE-{request.user.username}"
    )
