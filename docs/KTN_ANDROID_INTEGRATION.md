...
5. **User now has internet access** — all traffic routed securely through the KTN server
...
**KTN (Kitonga Network)** is a paid internet access service. When a user pays, they receive a **private encrypted tunnel** that gives them direct routed internet access — even when they are **outside a WiFi hotspot area**. Under the hood, KTN uses the WireGuard protocol to create this secure tunnel.

**The flow:** User pays → Gets private encrypted tunnel → Direct routed internet.
