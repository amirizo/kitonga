...
async function updateActivUsersDisplay(routerId) {
  ...
  if (data.success) {
    ...
    const totalDownload = data.active_users.reduce((sum, user) => sum + parseInt(user.bytes_in), 0)
    const totalUpload = data.active_users.reduce((sum, user) => sum + parseInt(user.bytes_out), 0)
