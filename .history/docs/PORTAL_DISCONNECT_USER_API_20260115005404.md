...
async function disconnectUser(routerId, username, macAddress) {
  const response = await fetch(
    `/api/portal/router/${routerId}/disconnect-user/`,
    {
      method: 'POST',
      headers: {
      ...
      <tbody>
        {users.map(user => (
          <tr key={user.session_id}>
            ...
            <td>{user.uptime}</td>
            <td>
              {user.database_info?.has_active_access ? (
            ...
            <td>
              <button
                onClick={() => handleDisconnect(user.username, user.mac_address)}
                disabled={disconnecting[user.username]}
...
class RouterUserManager {
  ...
  async disconnectUser(username, macAddress = null) {
    const response = await fetch(
      `/api/portal/router/${this.routerId}/disconnect-user/`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
  ...
  async disconnectExpiredUsers() {
    ...
    const expiredUsers = activeData.active_users.filter(
      user => user.database_info && !user.database_info.has_active_access
    );
