...
def redeem_voucher(request):
    ...
    if router_id and not tenant:
        try:
            router = Router.objects.get(id=router_id, is_active=True)
            tenant = router.tenant
            logger.info(f"Voucher redemption: Tenant resolved from router_id {router_id}: {tenant.slug}")
        except Router.DoesNotExist:
...
def list_bundles(request):
    ...
    if router_id and not tenant:
        try:
            router = Router.objects.get(id=router_id, is_active=True)
            tenant = router.tenant
            logger.info(f"list_bundles: Tenant resolved from router_id {router_id}: {tenant.slug}")
        except Router.DoesNotExist:
