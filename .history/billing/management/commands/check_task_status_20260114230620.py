...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if expired_users.exists():
            ...
            for user in expired_users[:10]:  # Show first 10
                ...
                for device in devices:
                    ...
                    self.stdout.write(
                        f"      Device: {device.mac_address} -> {router_info}"
                    )
        ...
        active_users = User.objects.filter(is_active=True, paid_until__gt=now).count()
        ...
        if expired_users.exists():
            self.stdout.write(self.style.ERROR("1. ⚠️  SET UP CRON JOB IMMEDIATELY!"))
            self.stdout.write(
