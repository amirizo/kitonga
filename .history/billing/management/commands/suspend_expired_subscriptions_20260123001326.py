...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Checking for expired subscriptions (grace_days={grace_days})...")
        result = suspend_expired_subscriptions(grace_days=grace_days)
        self.stdout.write("Summary:")
        self.stdout.write(str(result))
        if result.get("errors"):
            self.stderr.write("Some errors occurred during suspension. Check logs for details.")
        else:
