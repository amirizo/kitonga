...
class AutoSMSService:
    ...
    @classmethod
    def trigger_event_campaigns(
        cls, tenant, event_type: str, context: dict, user=None, payment=None
    ):
        """
        ...
        if (
            not tenant.subscription_plan
            or not tenant.subscription_plan.auto_sms_campaigns
        ):
            return
        ...
        campaigns = AutoSMSCampaign.objects.filter(
            tenant=tenant, trigger_type=event_type, status="active", is_active=True
        )
        ...
        for campaign in campaigns:
            try:
                ...
                result = sms_api.send_sms(
                    phone, message, reference=f"AUTO-{campaign.id}"
                )
                ...
                campaign.save(
                    update_fields=["total_sent", "total_failed", "last_triggered_at"]
                )
        ...
        for campaign in due_campaigns:
            ...
            for user in recipients:
                try:
                    ...
                    result = sms_api.send_sms(
                        user.phone_number, message, reference=f"SCHEDULED-{campaign.id}"
                    )
    ...
    def check_expiring_users(cls):
        ...
        expiring_campaigns = AutoSMSCampaign.objects.filter(
            trigger_type="access_expiring", status="active", is_active=True
        ).select_related("tenant", "tenant__subscription_plan")
...
def trigger_new_user_sms(user):
    ...
    AutoSMSService.trigger_event_campaigns(user.tenant, "new_user", context, user=user)
...
def trigger_payment_success_sms(payment):
    ...
    context = {
        ...
        "expiry_date": (
            payment.user.paid_until.strftime("%Y-%m-%d %H:%M")
            if payment.user.paid_until
            else ""
        ),
    }
...
def trigger_access_expired_sms(user):
    ...
    context = {
        ...
        "phone": user.phone_number,
        "expiry_date": (
            user.paid_until.strftime("%Y-%m-%d %H:%M") if user.paid_until else ""
        ),
    }
