            ...
            for session in active:
                ...
                try:
                    ...
                    db_info = {
                        ...
                        "access_expires_at": (
                            db_user.access_expires_at.isoformat()
...
def initiate_payment(request):
    ...
    if not tenant:
        ...
        logger.info(f"Payment initiated for tenant: {tenant.slug} ({tenant.business_name})")
    ...
    try:
        ...
        if created:
            ...
            logger.info(f"Found existing user {user.phone_number} for tenant {tenant.slug if tenant else 'GLOBAL'}")
    ...
    if bundle_id:
        try:
            ...
            return Response(
                {"success": False, "message": "Invalid bundle selected for this network"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    else:
        # Use default daily bundle (scoped to tenant)
        if tenant:
            bundle = Bundle.objects.filter(tenant=tenant, duration_hours=24, is_active=True).first()
        else:
            bundle = Bundle.objects.filter(duration_hours=24, is_active=True).first()
        
        if not bundle:
            return Response(
                {"success": False, "message": "No daily bundle configured for this network"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        amount = bundle.price
    ...
    # =========================================================================
    # MULTI-TENANT: Use tenant's ClickPesa credentials if configured
    # =========================================================================
    if tenant and tenant.clickpesa_client_id and tenant.clickpesa_api_key:
        # Use tenant's own ClickPesa account
        from .clickpesa import TenantClickPesaAPI
        clickpesa = TenantClickPesaAPI(tenant)
        logger.info(f"Using tenant {tenant.slug}'s ClickPesa credentials")
    else:
        # Use platform's ClickPesa account
        clickpesa = ClickPesaAPI()
        logger.info("Using platform ClickPesa credentials")
...
def redeem_voucher(request):
    ...
    try:
        ...
        if tenant:
            ...
            try:
                ...
                if Voucher.objects.filter(code=voucher_code).exists():
                    logger.warning(f"Voucher {voucher_code} exists but not for tenant {tenant.slug}")
                    return Response(
                        {"success": False, "message": "Voucher not valid for this network"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
        ...
        try:
            user, created = get_or_create_user(phone_number, tenant=voucher_tenant, max_devices=1)
            if created:
                logger.info(
...
def list_bundles(request):
    ...
    if tenant:
        ...
        logger.debug(f"Listing bundles for tenant {tenant.slug}: {bundles.count()} found")
    else:
