...
class TenantNextSMSAPI:
    ...
    def __init__(self, tenant):
        ...
        self.sender_id = tenant.nextsms_sender_id or tenant.business_name[:11]  # Max 11 chars for sender ID
        self.base_url = getattr(tenant, 'nextsms_base_url', None) or settings.NEXTSMS_BASE_URL
    ...
    def send_sms(self, phone_number, message, reference=None):
        ...
        try:
            ...
            logger.error(f'Tenant {self.tenant.slug}: Failed to send SMS to {phone_number}: {str(e)}')
            error_text = ''
    ...
    def send_bulk_sms(self, recipients, message, reference_prefix=None):
        ...
        for recipient in recipients:
            phone = recipient if isinstance(recipient, str) else recipient.get('phone_number', '')
            if not phone:
                continue
