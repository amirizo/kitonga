...
def disconnect_user(request, user_id):
    ...
    try:
        ...
        logger.info(f"Admin requested disconnect for user: {phone_number} (tenant: {tenant.slug if tenant else 'platform'})")
        ...
        for device in devices:
            try:
                        ...
                        try:
                            ...
                            if api:
                                ...
                                if result.get("success") or result.get("session_removed"):
                                    ...
                                    logger.info(f"Disconnected {device.mac_address} from router {router.name}")
                        except Exception as router_err:
                            logger.warning(f"Error disconnecting from router {router.name}: {router_err}")
                            disconnect_errors.append(f"{router.name}: {str(router_err)}")
                else:
                ...
                if device_disconnected:
                    ...
                    disconnect_errors.append(f'{device.mac_address}: Could not disconnect')
        ...
        if tenant_routers:
            for router in tenant_routers:
                try:
                    ...
                    if api:
                        from .mikrotik import disconnect_user_with_api
                        result = disconnect_user_with_api(api=api, username=phone_number, mac_address=None)
                        if result.get("success"):
                            logger.info(f'Cleanup disconnect for {phone_number} on router {router.name}')
                except Exception as cleanup_error:
                    logger.debug(f"Cleanup disconnect for {phone_number} on {router.name}: {cleanup_error}")
        else:
            try:
                result = disconnect_user_from_mikrotik(
