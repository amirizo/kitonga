...
class ClickPesaAPI:
    ...
    def get_account_balance(self):
        ...
        try:
            ...
            balances = {}
            balance_list = result if isinstance(result, list) else result.get('balances', [])
    ...
    def preview_mobile_money_payout(self, phone_number, amount, order_reference, currency='TZS'):
        """
    ...
    def create_mobile_money_payout(self, phone_number, amount, order_reference, currency='TZS'):
        """
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                return {
                    ...
                    'fee': payout.get('fee'),
                    'status': payout.get('status'),  # SUCCESS, PROCESSING, PENDING, FAILED, REFUNDED, REVERSED
...
class TenantClickPesaAPI(ClickPesaAPI):
    """
    Tenant-specific ClickPesa API client.
    
    Uses the tenant's own ClickPesa credentials instead of platform credentials.
    This allows each tenant to receive payments directly to their own account.
    """
    
    def __init__(self, tenant):
        """
        Initialize with tenant's ClickPesa credentials.
        
        Args:
            tenant: Tenant model instance with clickpesa_client_id and clickpesa_api_key
        """
        self.tenant = tenant
        self.client_id = tenant.clickpesa_client_id
        self.api_key = tenant.clickpesa_api_key
        self.base_url = getattr(settings, 'CLICKPESA_BASE_URL', 'https://api.clickpesa.com')
        self.token = None
        self.token_expires_at = None
        
        if not self.client_id or not self.api_key:
            logger.warning(f"Tenant {tenant.slug} has incomplete ClickPesa credentials")
