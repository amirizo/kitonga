...
def portal_user_hotspot_enable(request, user_id):
    ...
    if not routers.exists():
        return Response(
            {"success": False, "message": "No active routers configured for this tenant"},
            status=status.HTTP_400_BAD_REQUEST,
        )
        ...
        try:
            ...
            if action == "enable":
                ...
                if existing:
                    ...
                    router_result["success"] = True
                    router_result["detail"] = "User disabled on hotspot and sessions kicked"
                else:
                    router_result["success"] = True
                    router_result["detail"] = "User not found on hotspot (already absent)"
