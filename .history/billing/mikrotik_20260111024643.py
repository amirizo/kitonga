...
def grant_user_access_on_router(
    ...
    """
    result = {"user_created": False, "mac_bypassed": False, "errors": [], "router_id": router.id if router else None}
    ...
    try:
        ...
        try:
            ...
            logger.error(f"grant_user_access_on_router: create_hotspot_user failed for {username}: {e}")
        ...
        if mac_address:
            try:
                ...
                logger.error(f"grant_user_access_on_router: allow_mac failed for {mac_address}: {e}")
    ...
    try:
        ...
        if mac_address:
            try:
                ...
                logger.error(f"revoke_user_access_on_router: revoke_mac failed for {mac_address}: {e}")
                result["errors"].append(f"mac:{e}")
        ...
        if username:
            try:
                ...
                logger.error(f"revoke_user_access_on_router: disable user failed for {username}: {e}")
                result["errors"].append(f"user:{e}")
        ...
        try:
            ...
            logger.error(f"revoke_user_access_on_router: kick active session failed: {e}")
...
def authenticate_user_on_tenant_routers(
    ...
    for router in routers:
        ...
        try:
            ...
            logger.error(f"authenticate_user_on_tenant_routers: Failed on router {router.name}: {e}")
...
def revoke_user_access_on_tenant_routers(
    ...
    for router in routers:
        ...
        try:
            ...
            logger.error(f"revoke_user_access_on_tenant_routers: Failed on router {router.name}: {e}")
            result["router_results"].append({
                "router_id": router.id,
                "router_name": router.name,
                "router_host": router.host,
...
def force_immediate_internet_access_on_tenant_routers(
    ...
    for router in routers_list:
        ...
        try:
            api = get_tenant_mikrotik_api(router)
            if api is None:
                router_result["errors"].append(f"Cannot connect to router {router.host}:{router.port}")
                result["router_results"].append(router_result)
                continue
            ...
            try:
                ...
                try:
                    ...
                    if user_created:
                        logger.info(f"✓ Hotspot user created on {router.name} for {user.phone_number}")
                ...
                if mac_address:
                    try:
                        ...
                        if mac_ok:
                            logger.info(f"✓ IP binding created on {router.name} for {mac_address}")
                    except Exception as mac_error:
                        router_result["errors"].append(f"ip_binding: {mac_error}")
                
                # Consider success if either operation worked
                router_result["success"] = (
                    router_result["hotspot_user_created"] or 
                    router_result["ip_binding_created"]
                )
            ...
            logger.error(f"force_immediate_internet_access_on_tenant_routers: Failed on router {router.name}: {e}")
    ...
    if result["success"]:
        result["message"] = f"Internet access granted on {result['routers_succeeded']}/{result['routers_tried']} routers"
    ...
    for router in routers:
        ...
        try:
            ...
            try:
                ...
                router_result["session_removed"] = disconnect_result.get("session_removed", False)
                router_result["binding_removed"] = disconnect_result.get("binding_removed", False)
                router_result["user_disabled"] = disconnect_result.get("user_disabled", False)
            ...
            logger.error(f"disconnect_user_from_tenant_routers: Failed on router {router.name}: {e}")
            router_result["errors"].append(str(e))
...
def authorize_user_on_specific_router(
    ...
    if user.tenant and router.tenant:
        if user.tenant.id != router.tenant.id:
            result["message"] = f"Access denied: User belongs to tenant '{user.tenant.slug}', router belongs to '{router.tenant.slug}'"
            result["errors"].append("tenant_mismatch")
            logger.warning(f"SECURITY: Tenant mismatch! User {user.phone_number} ({user.tenant.slug}) tried to access router {router_id} ({router.tenant.slug})")
            return result
    elif user.tenant and not router.tenant:
        # Router has no tenant - might be a legacy/global router
        logger.warning(f"Router {router_id} has no tenant assigned, but user {user.phone_number} has tenant {user.tenant.slug}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to create hotspot user on {router.name}: {user_error}")
            ...
            if mac_address:
                try:
                    ...
                    logger.error(f"Failed to create IP binding on {router.name}: {mac_error}")
            
            # Success if either operation worked
            result["success"] = result["hotspot_user_created"] or result["ip_binding_created"]
            ...
            if result["success"]:
                ...
                logger.info(f"✓ User {user.phone_number} authorized on router {router.name} (ID: {router_id})")
            else:
        ...
        logger.error(f"authorize_user_on_specific_router: Failed on router {router.name}: {e}")
...
def revoke_user_on_specific_router(
    ...
    try:
        ...
        try:
            disconnect_result = disconnect_user_with_api(api, user.phone_number, mac_address)
        ...
        logger.error(f"revoke_user_on_specific_router: Failed on router {router.name}: {e}")
        result["errors"].append(str(e))
    ...
    try:
        ...
        if device_created:
            ...
            if active_devices > user.max_devices:
                ...
                device.save()
                logger.warning(
                    f"Device limit exceeded for {phone_number}: {active_devices}/{user.max_devices}"
                )
                device_info = {
                    "device_id": device.id,
                    "mac_address": mac_address,
