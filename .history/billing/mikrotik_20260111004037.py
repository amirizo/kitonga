...
def get_tenant_mikrotik_api(router, retries: int = 3, timeout: int = 10):
    ...
    for attempt in range(retries):
        try:
            ...
            logger.debug(f'Tenant MikroTik API connected to {host}:{port} on attempt {attempt + 1}')
            return api
            
        except Exception as e:
            last_error = e
            logger.warning(f'Tenant MikroTik connection attempt {attempt + 1}/{retries} to {host}:{port} failed: {e}')
            if attempt < retries - 1:
                import time
                time.sleep(1)
