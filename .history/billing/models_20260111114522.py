...
class SMSBroadcast(models.Model):
    ...
    def get_recipients(self):
        ...
        if self.target_type == 'all_users':
            ...
            users = User.objects.filter(phone_number__isnull=False).values('phone_number', 'name')
            recipients = list(users)
            
        elif self.target_type == 'all_tenants':
            # All tenant business phones
            tenants = Tenant.objects.filter(is_active=True).values('business_phone', 'business_name')
    ...
    def send_broadcast(self):
        ...
        for recipient in recipients:
            ...
            try:
                result = sms_api.send_sms(phone, self.message, reference=f'BROADCAST-{self.id}')
