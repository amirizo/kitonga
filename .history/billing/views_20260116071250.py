                ...
                try:
                    ...
                    if devices.exists():
                        ...
                        for device in devices:
                            try:
                                ...
                                if payment_router:
                                    ...
                                    logger.info(f"Granting access on specific payment router: {payment_router.name}")
                                elif user.tenant:
                        ...
                        mikrotik_auth_result = {
                            ...
                            "router_used": payment_router.name if payment_router else "all_tenant_routers",
                        }
                    else:
                        ...
                        if payment_router:
                            ...
                            from .mikrotik import get_tenant_mikrotik_api, create_hotspot_user_on_router, safe_close
                            try:
                                api = get_tenant_mikrotik_api(payment_router)
                                if api:
                                    try:
                                        create_hotspot_user_on_router(api, phone_number, phone_number)
                                        auto_access_result = {"success": True, "router": payment_router.name}
                                        logger.info(f"Created hotspot user on payment router {payment_router.name} for {phone_number}")
                                    finally:
                                        safe_close(api)
                                else:
                                    auto_access_result = {"success": False, "message": f"Cannot connect to router {payment_router.name}"}
                            except Exception as router_err:
                                logger.error(f"Error creating hotspot user on router {payment_router.name}: {router_err}")
                                auto_access_result = {"success": False, "message": str(router_err)}
                        elif user.tenant:
                        ...
                        mikrotik_auth_result = {
                            ...
                            "router_used": payment_router.name if payment_router else "tenant_routers" if user.tenant else "global",
                        }
                        logger.info(
