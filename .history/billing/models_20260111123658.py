...
# Tenant-level SMS Broadcasts
class TenantSMSBroadcast(models.Model):
    """
    Tenant-specific SMS campaigns created from tenant portal
    Simple and scoped to a single tenant
    """
    STATUS_CHOICES = [
        ("draft", "Draft"),
        ("pending", "Pending"),
    ...
    tenant = models.ForeignKey(Tenant, on_delete=models.CASCADE, related_name="portal_sms_broadcasts")
    title = models.CharField(max_length=150)
    message = models.TextField(max_length=320)
    created_by = models.ForeignKey(DjangoUser, on_delete=models.SET_NULL, null=True, blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="draft")

    # Targeting within tenant
    target_type = models.CharField(
        max_length=20,
        choices=[
            ("all_users", "All Users"),
    ...
    custom_recipients = models.JSONField(null=True, blank=True)
    ...
    def get_recipients(self):
        recipients = []
        if self.target_type == "all_users":
            users = self.tenant.wifi_users.filter(phone_number__isnull=False).values("phone_number", "name")
            recipients = list(users)
        elif self.target_type == "active_users":
            now = timezone.now()
            users = self.tenant.wifi_users.filter(phone_number__isnull=False, is_active=True, paid_until__gte=now).values("phone_number", "name")
            recipients = list(users)
        elif self.target_type == "expired_users":
            now = timezone.now()
            users = self.tenant.wifi_users.filter(phone_number__isnull=False, paid_until__lt=now).values("phone_number", "name")
            recipients = list(users)
        elif self.target_type == "custom":
    ...
    def send_broadcast(self):
        ...
        if not self.tenant or not self.tenant.is_active:
            return False, "Tenant not active"

        # Check tenant subscription allows SMS
        if not (self.tenant.subscription_plan and self.tenant.subscription_plan.sms_notifications):
            return False, "Tenant subscription does not include SMS notifications"

        # TODO: Optionally allow tenant to configure their own NextSMS credentials - use tenant.nextsms_* if provided
        sms_api = NextSMSAPI(
            username=self.tenant.nextsms_username or None,
            password=self.tenant.nextsms_password or None,
            sender_id=self.tenant.nextsms_sender_id or None,
        )
        ...
        if not recipients:
            self.status = "failed"
            self.error_message = "No recipients found"
            self.completed_at = timezone.now()
            self.save()
            return False, "No recipients"

        # Check tenant monthly allowance (from plan)
        allowance = getattr(self.tenant.subscription_plan, 'sms_monthly_allowance', None)
        if allowance is None:
            allowance = getattr(settings, 'DEFAULT_TENANT_SMS_ALLOWANCE', 1000)

        # Count tenant SMS sent this month
        from django.utils.timezone import now
        start_of_month = now().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        sent_this_month = SMSLog.objects.filter(tenant=self.tenant, sent_at__gte=start_of_month).count()

        if sent_this_month + len(recipients) > allowance:
            self.status = "failed"
            self.error_message = f"Tenant SMS monthly allowance exceeded ({sent_this_month}/{allowance})"
            self.save()
        ...
        for recipient in recipients:
            ...
            if not phone:
                continue
            try:
                result = sms_api.send_sms(phone, self.message, reference=f'TENANT-{self.tenant.slug}-{self.id}')
                SMSLog.objects.create(
                    tenant=self.tenant,
                    phone_number=phone,
                    message=self.message,
        ...
        self.completed_at = timezone.now()
        self.save()
        return True, f"Sent {self.sent_count}, failed {self.failed_count}"
