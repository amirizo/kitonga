...
def trigger_immediate_hotspot_login(phone_number, mac_address, ip_address):
    ...
    try:
        ...
        result = {
            "success": access.get("success", False),
            "message": "Access granted (bypassed / user created)" if access.get("success") else "Access provisioning failed",
            "method": "routeros_api",
...
def deactivate_user_other_devices(user, new_mac_address, new_ip_address=None):
    ...
    try:
        ...
        if not other_devices.exists():
            logger.info(f"No other active devices to deactivate for {user.phone_number}")
            return result
        ...
        tenant_routers = []
        if user.tenant:
            tenant_routers = list(Router.objects.filter(tenant=user.tenant, is_active=True))
                        ...
                        try:
                            ...
                            if revoke_result.get("success"):
                                result["revoked_macs"].append({
                                    "mac": old_mac,
                                    "router": router.name,
                                    "session_kicked": revoke_result.get("session_kicked", False)
                                })
                                logger.info(
                                    f"Revoked old device {old_mac} from router {router.name} for {user.phone_number}"
                                )
                        except Exception as router_err:
                            result["errors"].append(f"Router {router.name}: {str(router_err)}")
                            logger.warning(f"Failed to revoke {old_mac} from {router.name}: {router_err}")
                else:
                    # Legacy: use global router
                    try:
                        revoke_result = revoke_user_access(mac_address=old_mac, username=user.phone_number)
                        if revoke_result.get("success"):
                            result["revoked_macs"].append({
                                "mac": old_mac,
                                "router": "global",
                            ...
                            logger.info(f"Revoked old device {old_mac} from global router for {user.phone_number}")
                    except Exception as global_err:
                        result["errors"].append(f"Global router: {str(global_err)}")
                        logger.warning(f"Failed to revoke {old_mac} from global router: {global_err}")
        ...
        logger.error(f"Error in deactivate_user_other_devices for {user.phone_number}: {e}")
