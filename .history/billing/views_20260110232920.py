...
def list_payments(request):
    ...
    try:
        ...
        if payment_type in ['all', 'wifi']:
            ...
            if phone_filter:
                wifi_payments = wifi_payments.filter(phone_number__icontains=phone_filter)
            if parsed_date_from:
            ...
            wifi_total = wifi_payments.filter(status='completed').aggregate(total=Sum('amount'))['total'] or 0
            
            for payment in wifi_payments:
                payments_data.append({
                    'id': payment.id,
                    'payment_type': 'wifi',
                    'tenant': payment.user.tenant.slug if payment.user and payment.user.tenant else 'platform',
                    'tenant_name': payment.user.tenant.business_name if payment.user and payment.user.tenant else 'Platform',
        ...
        if payment_type in ['all', 'subscription']:
            subscription_payments = TenantSubscriptionPayment.objects.all().order_by('-created_at')
            
            if status_filter:
                subscription_payments = subscription_payments.filter(status=status_filter)
            if parsed_date_from:
                subscription_payments = subscription_payments.filter(created_at__gte=parsed_date_from)
            if parsed_date_to:
                subscription_payments = subscription_payments.filter(created_at__lte=parsed_date_to)
            if tenant_filter:
                subscription_payments = subscription_payments.filter(tenant__slug=tenant_filter)
            
            subscription_payments_count = subscription_payments.count()
            subscription_total = subscription_payments.filter(status='completed').aggregate(total=Sum('amount'))['total'] or 0
            ...
            for payment in subscription_payments:
                payments_data.append({
                    ...
                    'phone_number': payment.tenant.phone or '',
                    'amount': str(payment.amount),
        ...
        return Response({
            ...
            'pagination': {
                ...
                'page': page,
                'page_size': page_size,
                'total_pages': (total_payments + page_size - 1) // page_size if total_payments > 0 else 0
            },
            'summary': {
                'wifi_payments': {
...
def get_payment_detail(request, payment_id):
    ...
    try:
        ...
        if payment_type == 'subscription':
            ...
            payment_data = {
                ...
                'plan': {
                    ...
                    'price_monthly': str(payment.plan.price_monthly),
                    'price_yearly': str(payment.plan.price_yearly)
            ...
            payment_data = {
                ...
                'user': {
                    ...
                    'tenant': payment.user.tenant.slug if payment.user.tenant else 'platform'
        ...
        return Response({
            'success': False,
            'message': 'Wi-Fi payment not found'
        }, status=status.HTTP_404_NOT_FOUND)
    ...
    if mac_address:
        try:
            ...
            if device_tracking_result['success']:
                ...
                device = user.devices.get(mac_address=mac_address)
                logger.info(f'Device registered during payment initiation for {phone_number}: {mac_address}')
            else:
                logger.warning(f'Device tracking failed during payment initiation for {phone_number}: {device_tracking_result["message"]}')
                
        except Exception as device_error:
            logger.error(f'Device registration error during payment initiation for {phone_number}: {str(device_error)}')
            ...
            if mac_address:
                try:
                    ...
                    if device_tracking_result['success']:
                        ...
                        logger.info(f'Enhanced device tracking successful for voucher user {phone_number}: {mac_address}')
                    else:
                        ...
                        if device_tracking_result.get('device_limit_exceeded'):
                            device_info = {
                                ...
                                'device_count': device_tracking_result.get('active_devices', 0),
                                'max_devices': device_tracking_result.get('max_devices', user.max_devices),
                        ...
                        logger.warning(f'Enhanced device tracking failed for voucher user {phone_number}: {device_tracking_result["message"]}')
                ...
                try:
                    ...
                    if device_tracking_result['success']:
                        ...
                        if device_tracking_result.get('device_limit_exceeded'):
                            has_access = False
                            denial_reason = device_tracking_result['message']
                            logger.warning(f'Device limit exceeded for {username}: {device_tracking_result.get("active_devices", 0)}/{device_tracking_result.get("max_devices", user.max_devices)} (method: {access_method})')
                        else:
                            logger.warning(f'Device tracking failed for {username}: {device_tracking_result["message"]} (method: {access_method})')
                
                except Exception as tracking_error:
                    logger.error(f'Enhanced device tracking error for {username}: {str(tracking_error)} (method: {access_method})')
                    # Fallback to original device tracking method
                    device, created = Device.objects.get_or_create(
                        user=user,
                        mac_address=mac_address,
