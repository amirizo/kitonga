    ...
    if request.method == "GET":
        try:
            ...
            settings_data["activity"] = {
                "new_tenants_today": Tenant.objects.filter(created_at__date=today).count(),
                "new_users_today": User.objects.filter(created_at__date=today).count(),
                "payments_today": Payment.objects.filter(created_at__date=today, status='completed').count(),
                "subscription_payments_today": TenantSubscriptionPayment.objects.filter(created_at__date=today, status='completed').count(),
            }
            ...
            return Response(
                {"success": False, "message": f"Error retrieving system settings: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
def system_status(request):
    ...
    try:
        ...
        try:
            routers_online = Router.objects.filter(is_active=True, status='online').count()
        ...
        for tenant in Tenant.objects.filter(is_active=True):
            tenant_data = {
                ...
                "vouchers_available": Voucher.objects.filter(tenant=tenant, is_used=False).count(),
            }
...
def expiry_watcher_status(request):
    ...
    try:
        ...
        ).select_related('tenant').values("id", "phone_number", "paid_until", "tenant__slug", "tenant__business_name")
        ...
        tenant_stats = {}
        for tenant in Tenant.objects.filter(is_active=True):
            tenant_active = User.objects.filter(tenant=tenant, is_active=True, paid_until__gt=now).count()
            tenant_expiring = User.objects.filter(
                tenant=tenant, is_active=True, paid_until__gt=now, paid_until__lte=now + timedelta(minutes=30)
            ).count()
            tenant_expired = User.objects.filter(tenant=tenant, is_active=True, paid_until__lte=now).count()
