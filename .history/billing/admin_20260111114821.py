...
class SMSBroadcastAdmin(admin.ModelAdmin):
    ...
    list_display = ['title', 'target_type_display', 'status_badge', 'progress_display', 'created_by_name', 'created_at']
    list_filter = ['status', 'target_type', 'created_at']
    search_fields = ['title', 'message', 'created_by__username']
    readonly_fields = ['id', 'sent_count', 'failed_count', 'total_recipients', 'started_at', 'completed_at', 'error_message']
    ordering = ['-created_at']
    
    fieldsets = (
        ('Campaign Info', {
            'fields': ('title', 'message', 'target_type', 'target_tenant', 'custom_recipients')
        }),
        ('Status', {
            'fields': ('status', 'total_recipients', 'sent_count', 'failed_count', 'error_message')
        }),
        ('Timestamps', {
    ...
    def send_broadcast(self, request, queryset):
        ...
        for broadcast in queryset:
            if broadcast.status in ['draft', 'pending']:
                ...
                if success:
                    self.message_user(request, f"Broadcast '{broadcast.title}' sent successfully: {message}")
                else:
                    self.message_user(request, f"Broadcast '{broadcast.title}' failed: {message}", level='ERROR')
