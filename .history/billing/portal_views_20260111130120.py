...
def check_sms_broadcast_permission(tenant):
    ...
    if hasattr(tenant.subscription_plan, 'sms_broadcast') and tenant.subscription_plan.sms_broadcast:
        return True, None
    ...
    return False, "SMS broadcast is only available for Business and Enterprise plans. Please upgrade to unlock this feature."
    ...
    if request.method == 'GET':
        ...
        response_data = {
            ...
            'config': {
                ...
                'nextsms_sender_id': tenant.nextsms_sender_id or tenant.business_name[:11] if tenant.business_name else '',
                'nextsms_base_url': tenant.nextsms_base_url or 'https://messaging-service.co.tz',
        ...
        if 'nextsms_base_url' in serializer.validated_data and serializer.validated_data['nextsms_base_url']:
...
def portal_sms_broadcasts(request):
    ...
    from .serializers import TenantSMSBroadcastCreateSerializer, TenantSMSBroadcastSerializer
    ...
    if request.method == 'GET':
        broadcasts = TenantSMSBroadcast.objects.filter(tenant=tenant).order_by('-created_at')
...
def portal_sms_broadcast_send(request, broadcast_id):
    ...
    logger.info(f"Tenant {tenant.slug} sent SMS broadcast: {broadcast.title} - {message}")
...
def portal_sms_send_single(request):
    ...
    try:
        ...
        return Response({
            'success': result.get('success', False),
            'message': 'SMS sent successfully' if result.get('success') else result.get('message', 'Failed to send SMS')
        })
    except Exception as e:
