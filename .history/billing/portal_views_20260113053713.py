...
def portal_router_save_config(request):
    ...
    If use_vpn=True (default), automatically:
    - Assigns a VPN IP (10.100.0.X)
    - Generates WireGuard keys
    - Returns MikroTik commands to configure the router
    """
    tenant = getattr(request, "tenant", None)
    if not tenant:
        return Response(
    ...
    # Get use_vpn parameter (default True for new routers)
    use_vpn = serializer.validated_data.get("use_vpn", True)

    success, message, router = wizard.save_router_config(
        name=serializer.validated_data["name"],
        host=serializer.validated_data.get("host", "pending"),
        port=serializer.validated_data.get("port", 8728),
        username=serializer.validated_data.get("username", "admin"),
        password=serializer.validated_data.get("password", ""),
        use_ssl=serializer.validated_data.get("use_ssl", False),
    ...
    if success:
        ...
        # Include VPN configuration if VPN is enabled
        if router.vpn_enabled and router.vpn_ip:
            vpn_config = wizard.get_vpn_config(router)
            if vpn_config.get('success'):
                response_data["vpn_config"] = {
                    "vpn_ip": vpn_config["vpn_ip"],
                    "vps_endpoint": vpn_config["vps_endpoint"],
                    "server_public_key": vpn_config["server_public_key"],
                }
                response_data["mikrotik_commands"] = vpn_config["mikrotik_commands"]
                response_data["setup_instructions"] = [
                    "1. Copy the MikroTik commands below",
                    "2. Connect to your router via Winbox or SSH",
                    "3. Paste and run the commands",
                    f"4. Change the API password to: {router.password or 'your-secure-password'}",
                    "5. Return here and click 'Test VPN Connection'",
                ]
