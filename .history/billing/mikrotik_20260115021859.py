...
def disconnect_user_with_api(api, username: str, mac_address: str = None, block_user: bool = True) -> dict:
    ...
    try:
        ...
        # Step 1.5: Remove hotspot cookies to prevent auto-reconnection
        # Cookies allow users to reconnect without re-entering credentials
        try:
            cookies = api.get_resource("/ip/hotspot/cookie")
            all_cookies = cookies.get()
            
            for cookie in all_cookies:
                cookie_user = cookie.get("user", "")
                cookie_mac = cookie.get("mac-address", "")
                
                should_remove_cookie = False
                if username and cookie_user == username:
                    should_remove_cookie = True
                if target_mac and cookie_mac.upper() == target_mac.upper():
                    should_remove_cookie = True
                
                if should_remove_cookie:
                    try:
                        cookies.remove(id=cookie[".id"])
                        logger.info(f"🍪 Removed hotspot cookie for {username}: {cookie_mac}")
                    except Exception:
                        pass
                        
        except Exception as e:
            # Cookies might not exist or API might not support - not critical
            logger.debug(f"Could not remove hotspot cookies: {e}")
        ...
        # Step 3: Disable OR DELETE hotspot user (try multiple formats)
        # IMPORTANT: We DELETE the user to prevent cookie-based reconnection
        try:
            users = api.get_resource("/ip/hotspot/user")
            
            # Try different username formats
            for name_format in [username, username.replace("+", ""), f"+{username}"]:
                try:
                    user_list = users.get(name=name_format)
                    for user in user_list:
                        try:
                            # DELETE the user instead of just disabling
                            users.remove(id=user[".id"])
                            result["user_disabled"] = True  # Actually deleted
                            logger.info(f"🚫 DELETED hotspot user: {name_format}")
                        except Exception as del_err:
                            # If delete fails, try disable
                            try:
                                users.set(id=user[".id"], disabled="yes")
                                result["user_disabled"] = True
                                logger.info(f"🚫 Disabled hotspot user: {name_format}")
        ...
        # This MUST block the MAC from accessing the network entirely
        if block_user and target_mac:
            try:
                bindings = api.get_resource("/ip/hotspot/ip-binding")
                
                # First, remove ALL existing bindings for this MAC (including bypassed)
                existing = bindings.get(mac_address=target_mac)
                for binding in existing:
                    try:
                        bindings.remove(id=binding[".id"])
                        logger.info(f"🗑️ Removed existing binding for {target_mac}: type={binding.get('type')}")
                    except Exception:
                        pass
                
                # Now add a BLOCKED binding - this prevents login entirely
                # In MikroTik: type=blocked means the device cannot access hotspot at all
                bindings.add(
                    **{
                ...
                # ALSO: Try to disable any hotspot user that might exist
                # This double-ensures the user cannot login even with credentials
                try:
                    users = api.get_resource("/ip/hotspot/user")
                    # Search for user by various phone formats
                    for name_format in [username, username.replace("+", ""), f"+{username}"]:
                        try:
                            user_list = users.get(name=name_format)
                            for user in user_list:
                                users.set(id=user[".id"], disabled="yes")
                                result["user_disabled"] = True
                                logger.info(f"� Also disabled hotspot user: {name_format}")
                        except Exception:
                            pass
                except Exception:
                    pass
