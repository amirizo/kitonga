...
def list_sms_broadcasts(request):
    ...
    for b in broadcasts:
        data.append({
            ...
            'target_tenant_name': b.target_tenant.business_name if b.target_tenant else None,
            'status': b.status,
...
def create_sms_broadcast(request):
    ...
    valid_types = ['all_users', 'all_tenants', 'tenant_users', 'active_users', 'expired_users', 'custom']
    ...
    if target_type == 'custom':
        ...
        if len(custom_recipients) == 0:
            return Response({
                'success': False,
                'message': 'custom_recipients cannot be empty'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    result_message = f'Broadcast created successfully. {len(recipients)} recipients found.'
    if send_now:
...
def get_sms_broadcast(request, broadcast_id):
    ...
    sms_logs = SMSLog.objects.filter(
        sms_type='admin',
        message=broadcast.message,
        sent_at__gte=broadcast.created_at
    ...
    return Response({
        ...
        'broadcast': {
            ...
            'target_type': broadcast.target_type,
            'target_type_display': broadcast.get_target_type_display(),
            'target_tenant': str(broadcast.target_tenant.id) if broadcast.target_tenant else None,
            'target_tenant_name': broadcast.target_tenant.business_name if broadcast.target_tenant else None,
...
def preview_sms_broadcast(request):
    ...
    if target_type == 'all_users':
        users = WiFiUser.objects.filter(phone_number__isnull=False).values('phone_number', 'name')[:20]
        recipients = list(users)
        total = WiFiUser.objects.filter(phone_number__isnull=False).count()
        
    elif target_type == 'all_tenants':
        tenants = Tenant.objects.filter(is_active=True).values('business_phone', 'business_name')[:20]
        recipients = [{'phone_number': t['business_phone'], 'name': t['business_name']} for t in tenants]
        ...
        if target_tenant_id:
            try:
                ...
                recipients = list(users)
                total = WiFiUser.objects.filter(tenant=target_tenant, phone_number__isnull=False).count()
            except (Tenant.DoesNotExist, ValueError):
                total = 0
        else:
            total = 0
        ...
        recipients = list(users)
        total = WiFiUser.objects.filter(phone_number__isnull=False, is_active=True, paid_until__gte=now).count()
        ...
        recipients = list(users)
        total = WiFiUser.objects.filter(phone_number__isnull=False, paid_until__lt=now).count()
    else:
        total = 0
    ...
    return Response({
        'success': True,
        'target_type': target_type,
        'target_tenant_name': target_tenant.business_name if target_tenant else None,
...
def send_single_sms(request):
    ...
    sms_api = NextSMSAPI()
    result = sms_api.send_sms(phone_number, message, reference=f'SINGLE-{request.user.username}')
