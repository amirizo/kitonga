...
class RouterMonitor:
    ...
    def _get_system_resources(self, api):
        ...
        try:
            ...
            resources = resource.get() if hasattr(resource, 'get') else resource
            if resources:
                res = resources[0]
    ...
    def _get_hotspot_user_count(self, api):
        ...
        try:
            ...
            users = resource.get() if hasattr(resource, 'get') else resource
    ...
    def _get_interface_stats(self, api):
        ...
        try:
            ...
            interfaces = resource.get() if hasattr(resource, 'get') else resource
            if interfaces:
    ...
    def get_current_status(self):
        ...
        status = {
            ...
            "last_seen": (
                self.router.last_seen.isoformat() if self.router.last_seen else None
            ),
            "metrics": {},
        }
        ...
        try:
            ...
            if api:
                ...
                # Get resources - need to call .get() on the resource
                resource = api.get_resource("/system/resource")
                resources = resource.get() if hasattr(resource, 'get') else resource
                if resources:
                    res = resources[0]
                ...
                users = active_resource.get() if hasattr(active_resource, 'get') else active_resource
                status["metrics"]["active_users"] = len(users) if users else 0

                # Get interfaces
                iface_resource = api.get_resource("/interface")
                interfaces = iface_resource.get() if hasattr(iface_resource, 'get') else iface_resource
                if interfaces:
        ...
        results.append(
            {
                ...
                "error": (
                    snapshot.error_message if snapshot else "Failed to save snapshot"
                ),
            }
        )
