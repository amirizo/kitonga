...
class WebhookService:
    ...
    def dispatch_event(cls, tenant, event_type: str, data: dict):
        ...
        if (
            ...
            or not tenant.subscription_plan.webhook_notifications
        ):
        ...
        webhooks = TenantWebhook.objects.filter(
            tenant=tenant, is_active=True, status__in=["active", "failing"]
        )
    ...
    def deliver_webhook(cls, delivery):
        ...
        try:
            ...
            if 200 <= response.status_code < 300:
                ...
                logger.info(
                    f"Webhook delivered: {webhook.name} - {delivery.event_type}"
                )
            else:
                ...
                logger.warning(
                    f"Webhook failed: {webhook.name} - HTTP {response.status_code}"
                )
    ...
    def process_pending_retries(cls):
        ...
        pending = WebhookDelivery.objects.filter(
            status="retrying", next_retry_at__lte=now
...
def trigger_payment_success_webhook(payment):
    ...
    data = {
        ...
        "user": {
            ...
            "paid_until": (
                payment.user.paid_until.isoformat() if payment.user.paid_until else None
            ),
        },
    }
