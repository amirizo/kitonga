...
def disconnect_user_with_api(api, username: str, mac_address: str = None) -> dict:
    """
    Disconnect a user from MikroTik hotspot and DISABLE their hotspot user account.
    Used for tenant-specific router disconnection and expired user removal.
    
    Flow after disconnection:
    1. User connects to WiFi → gets redirected to login page (normal MikroTik behavior)
    2. User tries to login → REJECTED (user is disabled)
    3. Login page shows error → User must buy new access
    4. When user pays/redeems voucher → create_hotspot_user_on_router() RE-ENABLES them
