    ...
    def post_process(self, paths, dry_run=False, **options):
        ...
        for original_name, processed in super().post_process(paths, dry_run, **options):
            ...
            if isinstance(processed, Exception):
                if '.map' in str(processed) or 'MissingFileError' in type(processed).__name__:
                    # Log and skip missing source map files
                    print(f"⚠️  Skipping missing file: {original_name}")
                    yield original_name, original_name, True
                    continue
            yield original_name, processed[0] if isinstance(processed, tuple) else processed, True if isinstance(processed, tuple) else processed
